...
function addInitialCommentsForPlant(plantName) {
  const initialComments = [
    { name: 'Fernanda', comment: 'Minha trapoeraba roxa está exibindo folhas vibrantes e coloridas em meu jardim.' },
    { name: 'Gustavo#456', comment: 'As folhas da minha trapoeraba roxa são tão intensas e chamativas.' },
    { name: 'Ana', comment: 'A trapoeraba roxa é uma planta de fácil cultivo e traz um toque de exotismo ao meu ambiente.' },
    { name: 'Ricardo', comment: 'Estou adorando a combinação das flores  roxas da trapoeraba com outras plantas em meu canteiro.' },
    { name: 'Mariana', comment: 'A trapoeraba roxa é uma excelente escolha para quem busca uma planta de baixa manutenção com um visual impactante.' }
  ];
...
submitButton.addEventListener('click', function() {
  ...
  if (commentText.length >= 10) {
    ...
    const plantName = 'Trapoeraba'; // Nome da planta específica
