...
function addInitialCommentsForPlant(plantName) {
  const initialComments = [
    { name: 'JoséVit', comment: 'Minha bromélia está florescendo lindamente!' },
    { name: 'JuliaROSA', comment: 'Adoro a variedade de cores das folhas da bromélia.' },
    { name: 'Geraldo12', comment: 'A bromélia é uma planta perfeita para ambientes internos.' },
    { name: 'Serjãoaoao', comment: 'É importante evitar o acúmulo de água no centro da bromélia.' },
    { name: 'Barbie1211', comment: 'Estou pensando em transplantar minha bromélia para um vaso maior.' },
  ];
...
submitButton.addEventListener('click', function() {
  ...
  if (commentText.length >= 10) {
    ...
    const plantName = 'Bromélia'; // Nome da planta específica
