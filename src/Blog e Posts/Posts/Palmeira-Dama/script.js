...
function addInitialCommentsForPlant(plantName) {
  const initialComments = [
    { name: 'Fernando', comment: 'Minha palmeira cria um belo visual tropical em meu jardim.' },
  { name: 'Carolina@567', comment: 'As folhas da minha palmeira são tão elegantes e exuberantes.' },
  { name: 'Marcelo', comment: 'A palmeira é uma planta majestosa que traz uma atmosfera de praia para o meu espaço.' },
  { name: 'Ana', comment: 'Estou cuidando da minha palmeira para garantir um crescimento saudável e frondoso.' },
  { name: 'Mariana', comment: 'As palmeiras são perfeitas para criar sombra e um ambiente tranquilo em meu jardim.' }
  ];
...
submitButton.addEventListener('click', function() {
  ...
  if (commentText.length >= 10) {
    ...
    const plantName = 'Palmeira'; // Nome da planta específica
