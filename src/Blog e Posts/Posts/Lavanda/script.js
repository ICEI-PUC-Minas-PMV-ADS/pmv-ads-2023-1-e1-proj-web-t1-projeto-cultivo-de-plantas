...
function addInitialCommentsForPlant(plantName) {
  const initialComments = [
    { name: 'Camila', comment: 'Minha lavanda está perfumando todo o meu jardim.' },
  { name: 'Leonardo!23', comment: 'As flores da minha lavanda são tão delicadas e bonitas.' },
  { name: 'Isabela', comment: 'A lavanda é uma planta versátil, posso usar suas flores para fazer sachês perfumados.' },
  { name: 'Gustavo', comment: 'Estou cultivando lavanda para aproveitar seus benefícios terapêuticos.' },
  { name: 'Mariana@456', comment: 'A lavanda é uma planta de fácil cultivo e suas flores são encantadoras.' }
  ];
...
submitButton.addEventListener('click', function() {
  ...
  if (commentText.length >= 10) {
    ...
    const plantName = 'lavanda'; // Nome da planta específica
